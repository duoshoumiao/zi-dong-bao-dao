...
from .sql import RecordDao, SubscribeDao, TreeDao, ApplyDao, pcr_date  
        ...
        try:  
            ...
            for record in log_list[::-1]:  
                ...
                if (time := record["battle_end_time"]) > int(latest_time) and record["battle_type"] == 1:  
                    ...
                    # --- 按 PCR 日获取该玩家的刀数统计 ---  
                    pcr_day = pcr_date(time)  
                    pcr_day_ts = pcr_day.timestamp()  
                    if pcr_day_ts not in flag_count_cache:  
                        flag_count_cache[pcr_day_ts] = self.record.get_day_flag_counts(time)  
                    pid = str(pcrid)  
                    if pid not in flag_count_cache[pcr_day_ts]:  
                        flag_count_cache[pcr_day_ts][pid] = {'normal': 0, 'kill': 0, 'comp': 0}  
                    pc = flag_count_cache[pcr_day_ts][pid]  
                    kill_count = pc['kill']  
                    normal_slots = pc['normal'] + pc['kill']  
                    pending_comp = pc['kill'] - pc['comp']  
                    ...
                    if is_kill:  
                        ...
                        if remain_time == 90:  
                            if normal_slots < 3:  
                                ...
                                flag = 0.5  # 正刀已满，必定是补偿刀  
                        else:  
                            if pending_comp > 0:  
                                flag = 0.5  # 补偿刀  
                            else:  
                                flag = 0   # 没有待出补偿，实际是完整刀  
